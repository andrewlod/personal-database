...
class IngestURLRequest(BaseModel):
    ...
    processing_options: Optional[Dict[str, Any]] = Field(
        default=None, description="Optional processing overrides"
    )
...
async def ingest_uploaded_files(
    ...
    try:
        ...
        background_tasks.add_task(
            process_uploaded_files, file_paths, options, rag_service, temp_dir
        )
...
async def process_multiple_urls(
    urls: List[str], processing_options: Dict[str, Any], rag_service
):
    ...
    try:
        ...
        logger.info(
            f"Generated {len(all_chunks)} chunks from {len(parsed_docs)} documents"
        )
...
async def process_uploaded_files(
    ...
    try:
        logger.info(
            f"Starting background processing for {len(file_paths)} uploaded files"
        )
            ...
            try:
                ...
                if file_path_obj.suffix == ".json":
                    ...
                    if "text" not in doc_data:
                        logger.warning(
                            f"Skipping {file_path_obj.name}: missing 'text' field"
                        )
                        continue
                    ...
                    logger.warning(
                        f"Skipping unsupported file type: {file_path_obj.name}"
                    )
                    continue
                ...
                # Chunk the document
                chunking_strategy = processing_options.get(
        ...
        logger.info(
            f"Generated {len(all_chunks)} chunks from {len(file_paths)} uploaded files"
        )
