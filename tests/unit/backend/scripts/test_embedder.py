...
class TestWeaviateVectorDB:
    ...
    def test_init_with_api_key(self):
        ...
        db = WeaviateVectorDB(
            host="localhost", port=8080, scheme="http", api_key="test-key"
        )
...
class TestEmbedder:
    ...
    def test_initialize_vector_db_success(
        self, mock_weaviate_db, mock_sentence_transformer
    ):
        ...
        mock_weaviate_db.assert_called_once_with(
            host="localhost", port=8080, scheme="http", api_key=None
        )
        mock_db_instance.initialize.assert_called_once()

    @patch("sentence_transformers.SentenceTransformer")
    @patch("scripts.embedder.WeaviateVectorDB")
    def test_initialize_vector_db_failure(
        self, mock_weaviate_db, mock_sentence_transformer
    ):
    ...
    @patch("sentence_transformers.SentenceTransformer")
    @patch("scripts.embedder.WeaviateVectorDB")
    def test_add_documents_no_valid_texts(
        self, mock_weaviate_db, mock_sentence_transformer
    ):
    ...
    def test_search_success(self, mock_weaviate_db, mock_sentence_transformer):
        ...
        mock_db_instance.search_vectors.assert_called_once_with(
            query_vector=[0.1, 0.2, 0.3], limit=5, score_threshold=0.7
        )
